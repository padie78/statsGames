...
export class DynamoDbEvolutionAiReportRepository implements IEvolutionAiReportRepository {
  private readonly tableName: string;

  constructor(tableName = process.env['TABLE_NAME'] ?? '') {
    if (!tableName) {
      throw new Error('DynamoDbEvolutionAiReportRepository: TABLE_NAME no configurado.');
    }
    this.tableName = tableName;
  }

  async save(report: EvolutionAiReportRecord): Promise<void> {
    ...
    await client.send(
      new PutCommand({
        ...
        Item: {
          ...
          SK: DynamoKeys.evolutionAiReportSk(report.periodId, report.platform),
          entityType: EntityType.EvolutionAiReport,
    ...
    const result = await client.send(
      new GetCommand({
        ...
        Key: {
          ...
          SK: DynamoKeys.evolutionAiReportSk(periodId, platform),
        },
      }),
    );
    ...
    const result = await client.send(
      new QueryCommand({
        ...
        ExpressionAttributeValues: {
          ':pk': DynamoKeys.userPk(userId),
          ':skPrefix': DynamoKeys.evolutionAiReportSkPrefix(),
        },
...
function mapItem(item: Record<string, unknown>): EvolutionAiReportRecord {
  return {
    ...
    verdict: String(item['verdict'] ?? 'stable'),
    ...
    status: (item['status'] as EvolutionAiReportRecord['status']) ?? 'failed',
